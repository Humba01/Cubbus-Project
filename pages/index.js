import React from 'react';
import Image from 'next/image';

function wcube() {
  return (
  <div>
    <html>
      <head>
        <meta charset="UTF-8"/>
        <meta lang="pt-br"/>
        <title>Humbanew WCube | O incrível jogo dos cubos matemáticos.</title>
        <link rel="icon" href="./assets/logo/new-logo/Humbanew Logo (Design) - 2.0 - 12º Gen - Branco.svg"/>
      </head>
      <body>
        <header class="cabecalho_conteiner">
          <div class="cabecalho_cont_parent" id="btn_main">
            <Image src="./assets/logo/new-logo/Humbanew Logo (Design) - 2.0 - 12º Gen - Teste.svg"/>          
          </div>
          <div class="cab_conteiner_title">
            <h1 class="text1_title">Humbanew WCube</h1>
          </div>
        </header>

        <section class="unote_slogan">
          <p class="unote_text_slogan" id="slogan">O incrível jogo dos cubos matemáticos.</p>
        </section>

      </body>
      <footer>
        <section class="rodape">
          <h1 class="rodape_titulo">Humbanew © 2015 - 2021</h1>
        </section>
      </footer>
    </html>
  </div>
  )
}

export default wcube;
